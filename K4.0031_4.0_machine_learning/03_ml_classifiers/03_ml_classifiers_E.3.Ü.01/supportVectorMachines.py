# ============================================================
# 3.Ü.01 - Support Vector Machines
# Goal: Classify gender based on physical facial measurements
# Dataset: gender_classification.csv (5001 rows, 8 columns)
# ============================================================


# -------------------- IMPORTS --------------------
# pandas: load and work with our CSV data as a table
# svm: the Support Vector Machine algorithm from sklearn
...
features = gender_data.drop(columns=["gender"])  # Input: 7 physical measurements
labels = gender_data["gender"]  # Output: 0 (Female) or 1 (Male)
...
# "linear" = find the straight boundary that best separates the two groups
svm_classifier = svm.SVC(kernel="linear")

# Train the model — it learns the pattern from the training data
svm_classifier.fit(feature_train, label_train)

# Make predictions on the test data (data the model has never seen)
label_predictions = svm_classifier.predict(feature_test)
...
precision = metrics.precision_score(label_test, label_predictions)
